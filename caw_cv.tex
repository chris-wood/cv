%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Long Professional Curriculum Vitae
% LaTeX Template
% Version 1.1 (9/12/12)
%
% This template has been downloaded from:
% http://www.latextemplates.com
%
% Original author:
% Rensselaer Polytechnic Institute (http://www.rpi.edu/dept/arc/training/latex/resumes/)
%
% Important note:
% This template requires the res.cls file to be in the same directory as the
% .tex file. The res.cls file provides the resume style used for structuring the
% document.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\documentclass[10pt]{res} % Use the res.cls style, the font size can be changed to 11pt or 12pt here

\def\labelitemi{--}

\usepackage{enumerate}
\usepackage{url}
\usepackage{helvet} % Default font is the helvetica postscript font
% \usepackage{newcent} % To change the default font to the new century schoolbook postscript font uncomment this line and comment the one above

\newsectionwidth{0pt} % Stops section indenting

\begin{document}

%----------------------------------------------------------------------------------------
%	YOUR NAME AND ADDRESS(ES) SECTION
%----------------------------------------------------------------------------------------

\name{Christopher A. Wood\\ \\} % Your name at the top

\address{{\bf Permanent Address} \\ 1114 Corella \\ Newport Beach, CA 92660 } % Your address 1
%{\bf Contact Information} \\ 
\address{Phone: (315) 806-5939 \\ Email: woodc1@uci.edu \\ \url{www.christopher-wood.com}} % Your address 2

%----------------------------------------------------------------------------------------

\begin{resume}

%----------------------------------------------------------------------------------------
%	OBJECTIVE SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{RESEARCH INTERESTS}}

\vspace{8pt} % Gap between title and text

Content-centric networking architectures and protocols, computer and network security and privacy, cryptographic algorithms and engineering, software engineering, and heterogeneous computing. \\ 

%----------------------------------------------------------------------------------------
%	EDUCATION SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{EDUCATION}} 

\vspace{8pt} % Gap between title and text

{\sl Doctor of Philosophy}, 
Computer Science \\ 
University of California Irvine, Irvine, CA \hfill 2013 - 2018 (expected) \\ 
Advisors: Dr. Gene Tsudik \\
GPA: 4.0/4.0

{\sl Master of Science}, Computer Science\\ 
Rochester Institute of Technology, Rochester, NY \hfill 2012 - 2013 \\
Thesis: Large Substitution Boxes with Efficient Combinational Implementations \\
Advisor: Dr. Stanis{\l}aw Radziszowski \\
GPA: 4.0/4.0
 
{\sl Bachelor of Science}, Computer Science and Software Engineering\\ 
Rochester Institute of Technology, Rochester, NY \hfill 2008 - 2012 \\
Concentrations: Computational Mathematics and Computer Engineering \\
Minor: Mathematics\\
GPA: 3.98/4.0 (Professional Field of Study GPA: 4.0/4.0)

%----------------------------------------------------------------------------------------
%	PUBLICATIONS SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{PUBLICATIONS}} 

\subsubsection*{Journal Articles}

\begin{enumerate}[J-1.]

\item P. Bajorski, A. Kaminsky, M. Kurdziel, M. Lukowiak, S. Radziszowski, and C. Wood, ``Stochastic Analysis and Modeling of a Tree-Based Group Key Distribution Method in Tactical Wireless Networks,'' to appear in {\it Journal of Telecommunications System \& Management, Engineering Journals, OMICS Publishing Group}.

\item C. A. Wood and J. Jacob, ``Characterization of Small Trees Based on their L(2,1)-Span,'' to appear in the \emph{AKCE International Journal of Graphs and Combinatorics}.

\item M. Lukowiak, S. Radziszowski, J. Vallino, C. Wood, ``Cybersecurity Education: Bridging the Gap between Hardware and Software Domains,'' {\it ACM Transactions on Computing Education}, 14(1) (2014).

\end{enumerate}
\vspace{-15pt}
\subsubsection*{Conference Proceedings}
\begin{enumerate}[C-1.]

\item J. Kurihara, C. A. Wood, and E. Uzun, ``An Encryption-Based Access Control Framework for Content-Centric Networking,'' \emph{to appear in IFIP Networking 2015}.

\item S. Skalicky, S. Lopez, M. Lukowiak, and C. A. Wood, ``Mission Control: A Performance Metric and Analysis of Control Logic for Pipelined Architectures on FPGAs,'' to appear in {\it Proceedings of the 2014 International Conference on Reconfigurable Computing and FPGAs - ReConFig 2014}, Cancun, Mexico. December 2014.

\item C. A. Wood and E. Uzun, ``Flexible End-to-End Content Security in CCN,'' \emph{IEEE Consumer Communications and Networking Conference (CCNC 2014) Special Session: Information Centric Networking}, Las Vegas, Nevada. January 2014.

\item S. Skalicky, C. A. Wood, M. Lukowiak, and M. Ryan, ``High Level Synthesis: Where Are We? A Case Study on Matrix Multiplication,'' in {\it Proceedings of the 2013 International Conference on Reconfigurable Computing and FPGAs - ReConFig 2013}, Cancun, Mexico. December 2013.

\item M. Lukowiak, A. Meneely, S. Radziszowski, J. Vallino, and C. Wood, ``Developing an Applied, Security-Oriented Computing Curriculum,'' in {\it Proceedings of the ASEE 2012}, San Antonio, Texas. June 2012.

\item C. A. Wood, ``Chaos-Based Symmetric Key Cryptosystems,'' in {\it Proceedings of the 2011 International Conference on Security \& Management}, Las Vegas, Nevada. July 2011.

\item C. A. Wood and R. K. Raj, ``Keyloggers in Cybersecurity Education,'' in {\it Proceedings of the 2010 International Conference on Security \& Management}, Las Vegas, Nevada. July 2010.

\end{enumerate}

\vspace{-15pt}
\subsubsection*{Theses}
\begin{enumerate}[T-1.]
	\item C. A. Wood, ``Large Substitution Boxes with Efficient Combinational Implementations,'' M.S. Thesis, Computer Science, Rochester Institute of Technology, Rochester, NY. August 2013.
\end{enumerate}

\vspace{-15pt}
\subsubsection*{Surveys}
\begin{enumerate}[S-1.]
	\item C. A. Wood, ``Small Folkman Numbers.'' \emph{Draft available online:} \url{http://christopher-wood.com/papers/FolkmanSurvey.pdf}.
\end{enumerate}

%----------------------------------------------------------------------------------------

% \vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	PRESENTATIONS SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{PRESENTATIONS AND POSTERS}} 

\vspace{18pt} % Gap between title and text

\begin{enumerate}[P-1.]

\item ``Efficient Security Bindings for Information Centric Networks,'' presentation, \emph{CCNxCon 2015, Palo Alto Research Center, Palo Alto, CA}. May 20, 2015.

\item ``Handling Trust Enforcement,'' presentation, \emph{CCNxCon 2015, Palo Alto Research Center, Palo Alto, CA}. May 20, 2015.

\item ``Secure Name Configuration and Prefix Registration in CCN,'' poster, \emph{CCNxCon 2015, Palo Alto Research Center, Palo Alto, CA}. May 19, 2015.

\item ``On the $L(2,1)$ Labeling of Trees,'' with Jobby Jacob (presenter), presentation, \emph{Joint Mathematics Meetings}, Baltimore, MD. January 15-18, 2014.

\item ``Secure Content Dissemination in Content Centric Networking,'' presentation, \emph{CCNxCon 2013, Palo Alto Research Center, Palo Alto, CA}. September 5, 2013.

\item ``Characterization Results for the L(2,1)-Labeling Problem on Trees,'' presentation, \emph{AMS Sectional Meeting, RIT, Rochester, NY}. September 22, 2012.

\item ``Chaos-Based Symmetric Key Cryptosystems,'' presentation, \emph{RIT Graduate Research Symposium, RIT, Rochester, NY}. July 22, 2011.

\item ``Keyloggers in Cybersecurity Education,'' presentation, \emph{2010 International Conference on Security \& Management}, Las Vegas, Nevada. July 2010.

\item ``Layered Driver Rootkit Detection on Microsoft Windows PCs,'' poster, \emph{RIT Undergraduate Research Symposium, RIT, Rochester, NY}. August 24, 2009. 
\end{enumerate}
\vspace*{-16pt}

%----------------------------------------------------------------------------------------
 
\vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	PATENTS SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{PATENTS}} 

\vspace{18pt} % Gap between title and text

\begin{enumerate}[D-1.]

\item P. Bajorski, A. Kaminsky, M. Kurdziel, M. Lukowiak, S. P. Radziszowski, and C. Wood, ``Electronic Key Management Using PKI to Support Group Key Establishment in the Tactical Environment, '' U.S. Patent Number 8,873,759. October 28, 2014.

\end{enumerate}
\vspace*{-16pt}

\vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	PROFESSIONAL EXPERIENCE SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{PROFESSIONAL EXPERIENCE}} 

\vspace{8pt} % Gap between title and text

{\sl Palo Alto Research Center} \hfill September 2014 - present \\
Computer Science Laboratory, Palo Alto, CA \hfill Network Software Development Engineer
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Develop the CCNx 1.0 software stack and APIs.
\item Implement internal code measurement tools for quantifiable software quality improvements.
\item Write IETF RFC drafts for various elements of the CCN protocol.
\item Contribute to CCN-related patent portfolio.
\end{itemize}

{\sl Palo Alto Research Center} \hfill June 2014 - September 2014 \\
Computer Science Laboratory, Palo Alto, CA \hfill Security and Privacy Research Intern
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Designed flexible manifest-based access control framework for CCNx 1.0.
\item Designed and implemented network-layer trust enforcement mechanics in CCNx 1.0.
\item Implemented various encryption-based access control primitives, including Broadcast Encryption and Proxy Re-Encryption for CCNx 1.0.
\end{itemize}

% {\sl Cigital, Inc.} \hfill March 2014 - present \\
% Dulles, MD \hfill Security Consultant Contractor
% \begin{itemize} \itemsep -2pt % Reduce space between items
% \item Focus on security-oriented C/C++ source code review and application architectural analyses.
% \item Educate developers and application stakeholders about security-oriented tasks in the SDLC.
% \end{itemize}

{\sl Palo Alto Research Center} \hfill July 2013 - September 2013 \\
Computer Science Laboratory, Palo Alto, CA \hfill Security and Privacy Research Intern
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Researched security and privacy aspects related to content-centric network (CCN).
\item Implemented the Green-Ateniese (pairing-based) and Chow-Weng-Yang-Deng (Schnorr- and ElGamal-based) Proxy Re-Encryption schemes in Java for use in a CCNx application.
\item Studied and tested various techniques for securing content that is distributed throughout a CCN mesh for confidentiality purposes.
\item Experimented with techniques for improving name privacy in CCN.
\end{itemize}

{\sl Intel Corporation} \hfill June 2012 - August 2012 \\
Virtual \& Parallel Computing Group, Folsom, CA \hfill Graphics Software Engineer Intern
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Developed production features for tool that processes hardware specifications to generate web content and source code for VHDL and C/C++ testbeds.
\item Interacted with internal customers within the VPG to utilize debug tools and environments for architecture specification and post-silicon testing.
\end{itemize}

{\sl L-3 Communications} \hfill March 2011 - August 2011 \\
Victor, NY \hfill Software Engineer Intern
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Designed and implemented a library and supporting drivers for the $\mu$-blox NEO5/6 GPS receiver driven by an Analog Devices Blackfin processor.
\item Extended an existing FAT file system driver to add support for SD devices.
\item Improved functionality of a CPLD controller for an embedded power supply.
\end{itemize}

{\sl Rochester Software Associates} \hfill November 2010 - March 2011 \\
Rochester, NY \hfill Software Engineer Intern
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Led the design, development, and documentation efforts for a new printer job management application that would service any number of jobs from clients across the network.
\item Tested and debugged an existing .NET implementation of an LPD client.
\end{itemize}

{\sl C Speed, LLC} \hfill May 2010 - August 2010 \\
Liverpool, NY \hfill Software Engineer Intern
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Designed and implemented an internal manufacturing part supply management system.
\item Implemented embedded firmware features and test routines in C, C++, and assembly for Coldfire V2 processors.
\end{itemize}

%----------------------------------------------------------------------------------------

% \vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	ACADEMIC EXPERIENCE SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{ACADEMIC EXPERIENCE}} 

\vspace{8pt} % Gap between title and text

{\sl Advanced Cryptography} \hfill May 5, 2015 \\
Guest Lecturer for Dr. Stanis{\l}aw Radziszowski (CS) \hfill (RIT)
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Lectured about digital signature algorithms, ElGamal and ECDSA batch verification techniques, standard public key infrastructures, and the OMC and ECQV implicit certificate schemes.
\end{itemize}

{\sl Cryptography II} \hfill April 8, 2013 \\
Guest Lecturer for Dr. Stanis{\l}aw Radziszowski (CS) \hfill (RIT)
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Lectured about recent research on the security and (hardware) implementation efficiency of cryptographic S-boxes.
\end{itemize}

{\sl Hardware and Software Design with Cryptographic Applications} \hfill February 2011 - May 2013 \\
Teaching Assistant and Lecturer for Dr. Marcin Lukowiak (CE) \hfill (RIT)
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Developed and delivered lecture material on cryptography, embedded software optimization techniques, the Impulse C high-level synthesis tool, and AES cache timing attacks.
\item Assisted students with weekly assignments and graded lab and project deliverables.
\end{itemize}

{\sl Computer Science I, II, and IV} \hfill January 2009 - May 2013 \\
Student Lab Assistant and Grader \hfill (RIT)
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Proctored problem solving sessions and ran lab meetings with lectures of weekly material.
\item Graded weekly lab assignments and midterm examinations.
\end{itemize}

{\sl Personal Software Engineering} \hfill December 2011 - March 2012 \\
Teaching Assistant for Professor Tom Reichlmayr (SE) \hfill (RIT)
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Assisted students with in-class programming assignments and course projects.
\item Graded projects written in C/C++ and Ruby (with Ruby on Rails).
\end{itemize}

{\sl Engineering of Software Subsystems} \hfill September 2011 - December 2011 \\
Teaching Assistant for Dr. James Vallino (SE) \hfill (RIT)
\begin{itemize} \itemsep -2pt % Reduce space between items
\item Assisted students with in-class exercises and unit questions based on a subset of the design patterns taught during the course.
\item Spent time with each student team to discuss course projects, including design decisions, application of design patterns, and alternatives considered.
\end{itemize}

%----------------------------------------------------------------------------------------

% \vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	COMPUTER SKILLS SECTION
%----------------------------------------------------------------------------------------

\vspace{8pt}
\section{\centerline{TECHNICAL SKILLS}}

\vspace{16pt} % Gap between title and text

\begin{itemize} \itemsep -2pt % Reduce space between items
\item Programming Languages: C/C++, C\#, Java, Python, Scala, Ruby, Assembly (MIPS), JavaScript, Objective-C, Standard ML, Scheme
\item Modeling Languages and Tools: VHDL, Verilog, UML, SPIN (with PROMELA), Alloy
\item Specialized Software: MATLAB, Mathematica, WEKA, Magma, Sage, LLVM
\item Markup Languages: \LaTeX, HTML(5), CSS3
\item Web Frameworks: NodeJS, Spring MVC, Ruby on Rails
\end{itemize}

%----------------------------------------------------------------------------------------

% \vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	MEMBERSHIPS SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{MEMBERSHIPS}} 

% \vspace{8pt} % Reduce space between section title and contents

\begin{center}
IEEE, Student Member \\
ACM, Student Member \\ 
SIAM, Student Member \\
IACR, Student Member 
\end{center}

%----------------------------------------------------------------------------------------

% \vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	HONORS SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{HONORS AND ACTIVITIES}} 

\vspace{16pt} % Reduce space between section title and contents

\begin{itemize} \itemsep -2pt
\item NSF GRFP fellowship recipient, 2014
\item RIT Honors Program, 2009 -- 2013
\item RIT Tau Beta Pi Engineering Honors Society, 2011 -- 2013
\item RIT Outstanding Undergraduate Student award, selected, Winter 2012
\item RIT Computer Science MS Student Delegate, selected, Winter 2012
\item Recipient of Golisano College Honors research assistantship stipend, Winter 2009/2010 
\item Recipient of Golisano College Honors research assistantship stipend, Spring 2011
\item Recipient of RIT undergraduate research award stipend, Summer 2009
\item RIT Golisano College Dean's List, 2008 -- 2013
\item Student mentor for the FIRST LEGO League team hosted by RIT, Fall 2009 -- Winter 2010
\item Rochester Foodlink volunteer, Winter 2012/2013 -- March 2013
\item Society of Software Engineers, member, Fall 2008 -- Winter 2009/2010
\item RIT Electronic Gaming Society, member, Fall 2008 -- Spring 2010
\item RIT Intramural Flag Football Team, member, Fall 2010
\end{itemize}

%----------------------------------------------------------------------------------------

% \vspace{0.2in} % Some whitespace between sections

%----------------------------------------------------------------------------------------
%	INTERESTS SECTION
%----------------------------------------------------------------------------------------

\section{\centerline{INTERESTS}} 

\vspace{-5pt} % Reduce space between section title and contents

\begin{center}
Guitar, running, cycling, swimming, weightlifting, languages, and the natural sciences.
\end{center} 

%----------------------------------------------------------------------------------------

\end{resume} 
\end{document}